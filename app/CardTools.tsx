...
export function CardTools() {
  ...
  useEffect(() => {
    ...
    let tone = "";    try { tone = localStorage.getItem(storageKey("surface")) || ""; } catch { tone = ""; }    if (tone) { setSurface(tone); document.documentElement.style.setProperty("--larsa-surface", tone); document.documentElement.style.setProperty("--larsa-sidebar", deepen(tone)); }    let density = "";    try { density = localStorage.getItem(storageKey("density")) || ""; } catch { density = ""; }    if (density === "compact") { setDense(true); document.documentElement.setAttribute("data-density", "compact"); }    if (stored) {
      setAccent(stored);
      document.documentElement.style.setProperty("--larsa-accent", stored);
    }
  }, []);

const SURFACES = [ { id: "default", label: "Default", value: "" }, { id: "warm", label: "Warm", value: "#faf7f2" }, { id: "cool", label: "Cool", value: "#f4f7fb" }, { id: "mint", label: "Mint", value: "#f3f9f5" }, { id: "rose", label: "Rose", value: "#fbf5f6" }, { id: "grey", label: "Grey", value: "#f2f3f5" }, { id: "sand", label: "Sand", value: "#ece3d5" }, { id: "stone", label: "Stone", value: "#e6e3dd" }, { id: "sky", label: "Sky", value: "#dfe9f6" }, { id: "sage", label: "Sage", value: "#dfe9e0" }, { id: "blush", label: "Blush", value: "#f2dfe2" }, { id: "steel", label: "Steel", value: "#dde2e9" }, { id: "lilac", label: "Lilac", value: "#e6e2f4" }, { id: "clay", label: "Clay", value: "#ecdfd8" }, { id: "slateblue", label: "Slate blue", value: "#d6dde7" }, { id: "graphite", label: "Graphite", value: "#d5d7db" } ];function chooseDense(value: boolean) {    setDense(value);    if (value) document.documentElement.setAttribute("data-density", "compact");    else document.documentElement.removeAttribute("data-density");    try { if (value) localStorage.setItem(storageKey("density"), "compact"); else localStorage.removeItem(storageKey("density")); } catch { /* ignore */ }  }  function deepen(hex: string): string {    const c = hex.replace("#", "");    if (c.length !== 6) return "#000000";    const rr = parseInt(c.slice(0, 2), 16) / 255;    const gg = parseInt(c.slice(2, 4), 16) / 255;    const bb = parseInt(c.slice(4, 6), 16) / 255;    const mx = Math.max(rr, gg, bb);    const mn = Math.min(rr, gg, bb);    const li = (mx + mn) / 2;    let hu = 0;    let sa = 0;    if (mx !== mn) {      const d = mx - mn;      sa = li > 0.5 ? d / (2 - mx - mn) : d / (mx + mn);      hu = mx === rr ? (gg - bb) / d + (gg < bb ? 6 : 0) : mx === gg ? (bb - rr) / d + 2 : (rr - gg) / d + 4;      hu = hu / 6;    }    const L = 0.1;    const S = Math.min(sa * 1.7, 0.34);    const q = L < 0.5 ? L * (1 + S) : L + S - L * S;    const pp = 2 * L - q;    const hue = (t: number) => {      let x = t;      if (x < 0) x += 1;      if (x > 1) x -= 1;      if (x < 1 / 6) return pp + (q - pp) * 6 * x;      if (x < 1 / 2) return q;      if (x < 2 / 3) return pp + (q - pp) * (2 / 3 - x) * 6;      return pp;    };    const to = (v: number) => Math.round(v * 255).toString(16).padStart(2, "0");    return "#" + to(hue(hu + 1 / 3)) + to(hue(hu)) + to(hue(hu - 1 / 3));  }    function chooseSurface(value: string) {    setSurface(value);    if (value) document.documentElement.style.setProperty("--larsa-surface", value);    if (value) document.documentElement.style.setProperty("--larsa-sidebar", deepen(value));        else { document.documentElement.style.removeProperty("--larsa-surface"); document.documentElement.style.removeProperty("--larsa-sidebar"); }    try { if (value) localStorage.setItem(storageKey("surface"), value); else localStorage.removeItem(storageKey("surface")); } catch { /* ignore */ }  }function onAccent(hex: string): string {    const clean = hex.replace("#", "");    if (clean.length !== 6) return "#ffffff";    const rr = parseInt(clean.slice(0, 2), 16);    const gg = parseInt(clean.slice(2, 4), 16);    const bb = parseInt(clean.slice(4, 6), 16);    const luma = (rr * 299 + gg * 587 + bb * 114) / 1000;    return luma > 165 ? "#17181b" : "#ffffff";  }    function chooseAccent(value: string) {
    setAccent(value);
